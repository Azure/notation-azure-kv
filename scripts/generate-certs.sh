...
function createSelfSignedPKCS12() {
    local certName="self-signed-pkcs12"
...
function createSelfSignedPEM() {
    local certName="self-signed-pem"
    local policy=$(cat << EOF
{
  "issuerParameters": {
    "name": "Self"
...
function importCAIssuedPKCS12CertificateChain() {
    local certName="imported-ca-issued-pkcs12"
    local unorderedCertName="imported-ca-issued-pkcs12-unordered"
    local policy=$(cat << EOF
{
  "keyProperties": {
    "exportable": false
...
function importCAIssuedPEMCertificateChain() {
    local certName="imported-ca-issued-pem"
    local unorderedCertName="imported-ca-issued-pem-unordered"
    local policy=$(cat << EOF
{
  "keyProperties": {
    "exportable": false
...
function createPKCS12CertificateChainWithCSR() {
    local certName="csr-ca-issued-pkcs12-chain"
    local policy=$(cat << EOF
{
  "issuerParameters": {
    "name": "Unknown"
...
EOF
    ...
    # download CSR
    local csr=$(az keyvault certificate pending show --vault-name $AKV_NAME --name $certName --query "csr" -o tsv)
    local csrPath=${certName}.csr
    printf -- "-----BEGIN CERTIFICATE REQUEST-----\n%s\n-----END CERTIFICATE REQUEST-----\n" $csr > ${csrPath}
    local signedCertPath="${certName}.crt"
...
function createPEMCertificateChainWithCSR() {
    local certName="csr-ca-issued-pem-chain"
    local policy=$(cat << EOF
{
  "issuerParameters": {
    "name": "Unknown"
